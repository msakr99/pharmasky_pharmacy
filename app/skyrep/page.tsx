...
export default function SkyRepPage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 flex flex-col">
      ...
      <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full pt-8">
        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto px-4 py-6" style={{ maxHeight: 'calc(100vh - 200px)' }}>
                ...
                <div className={`flex gap-4 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`flex-1 ${message.role === 'user' ? 'flex justify-end' : ''}`}>
                    <div className={`prose prose-sm max-w-none ${
                      message.role === 'user' 
                        ? 'bg-[#F4F4F4] dark:bg-gray-800 text-gray-900 dark:text-white rounded-3xl px-4 py-3 inline-block'
                        : 'text-gray-900 dark:text-gray-100'
                    }`}>
                      <p className="whitespace-pre-wrap leading-relaxed m-0 text-[15px]">{message.content}</p>
                    </div>
                  </div>
                  
                  {message.role === 'user' && (
                    <div className="flex-shrink-0 w-8 h-8 rounded-sm bg-[#FF6B35] flex items-center justify-center">
                      <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="sticky bottom-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 px-4 py-4">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="relative flex items-end gap-2">
              ...
              {/* Input Field */}
              <div className="flex-1 relative bg-white dark:bg-gray-800 rounded-3xl border border-gray-300 dark:border-gray-600 shadow-sm focus-within:border-gray-400 dark:focus-within:border-gray-500 transition-all">
                <textarea
                  ref={textareaRef}
                  value={inputMessage}
                  ...
                  className="w-full resize-none bg-transparent px-4 py-3 pr-4 pl-12 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none text-[15px] leading-relaxed"
                  rows={1}
                  disabled={isSending}
                  style={{
                    maxHeight: '200px',
              ...
              {inputMessage.trim() && (
                <button
                  onClick={handleSend}
                  disabled={isSending}
                  className="flex-shrink-0 w-10 h-10 bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 disabled:bg-gray-300 dark:disabled:bg-gray-600 text-white dark:text-gray-900 rounded-full transition-all flex items-center justify-center shadow-sm hover:shadow-md disabled:cursor-not-allowed"
                  title="إرسال"
                >
                  {isSending ? (
            ...
            <p className="text-center text-xs text-gray-500 dark:text-gray-500 mt-2">
              SkyRep قد يرتكب أخطاء. تحقق من المعلومات المهمة.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Recording Indicator */}
      {isRecording && (
        <div className="fixed top-20 left-1/2 transform -translate-x-1/2 bg-red-500 text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2 z-50 animate-pulse">
      ...
      {/* Call Indicator */}
      {isInCall && (
        <div className="fixed top-20 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2 z-50 animate-pulse">
